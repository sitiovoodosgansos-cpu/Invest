...
import { ShoppingCart, Search, Link2, AlertCircle, DollarSign, TrendingUp } from 'lucide-react';
import OrnabirdSync from '../components/OrnabirdSync';

// As vendas espelhadas do Ornabird, com o dono de cada uma resolvido.
//
// Morava embaixo do catalogo, na tela de Vitrine. Separada porque as duas
// metades respondem perguntas diferentes e sao usadas em momentos diferentes:
// o catalogo e "o que eu tenho pra vender", esta tela e "o que ja vendi e de
// quem e o dinheiro". Empilhadas, a segunda ficava a uma rolagem inteira de
// distancia num dia com muitos anuncios.
//
// O QUE ESTA TELA ACRESCENTA AO ORNABIRD
// --------------------------------------
// A venda chega com o lote de origem junto, entao o dono e resolvido com
// exatidao — em vez do palpite por texto na descricao (matchSaleToBird), que
// erra quando duas racas tem nomes parecidos.
//
// Nao ha acao nenhuma aqui: a venda acontece no Ornabird, fonte da verdade.
// Esta e tambem a tela onde se confere o que a ordem de pagamento vai pagar,
// entao o dono de cada venda segue a MESMA regra da ordem (dono na data da
...
export default function VendasOrnabird() {
  ...
  const rows = useMemo(() => sales.map(s => {
    ...
    // Dono NA DATA DA VENDA, nao o dono de hoje. Depois de uma transferencia de
    // titularidade, usar o dono atual faria esta tela e a ordem de pagamento
    // apontarem investidores diferentes pra mesma venda — e e aqui que a
    // conferencia acontece.
    ...
    return (
      <div className="animate-in">
        <div className="page-header">
          <h2>Vendas Ornabird</h2>
          <p>Vendas espelhadas do Ornabird</p>
        </div>
        <div className="empty-state">
          <ShoppingCart size={48} />
          <h3>Nada sincronizado ainda</h3>
          <p style={{ maxWidth: 460, margin: '0 auto' }}>
            Esta pagina espelha as vendas do Ornabird, ja vinculadas ao lote de origem.
            Aparecem aqui depois de sincronizar, com os lotes do Plantel vinculados.
          </p>
  ...
  return (
    <div className="animate-in">
      <div className="page-header">
        <h2>Vendas Ornabird</h2>
        <p>Vendas espelhadas do Ornabird — somente leitura</p>
        <OrnabirdSync />
      </div>

      <div className="stats-grid">
        <div className="stat-card">
          <div className="stat-card-icon purple"><ShoppingCart size={20} /></div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ marginTop: 2 }}>
              Essas vendas nao geram lucro para ninguem — e nao entram em ordem de
              pagamento nenhuma — enquanto o lote de origem no Ornabird nao for
              vinculado a uma linha do Plantel.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="filter-bar">
        ...
        {/* No portal do investidor a lista tem uma pessoa so — ele. Um filtro
            "Todos os investidores" ali nao filtra nada e ainda sugere que
            existe algo mais pra ver. */}
        {!somenteLeitura && (
          <select
            className="form-input"
